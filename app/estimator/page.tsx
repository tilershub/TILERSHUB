...
export default function Estimator(){
  const firstValue = (sizes as any)?.tileSizes?.[0]?.value ?? '600x600';
  const [inp, setInp] = useState<Inputs>({
    len_ft: 0,
    wid_ft: 0,
    area_sqft: 0,                  // optional; if > 0, it overrides len*wid
    tile_value: firstValue,
    tile_price_each: 0,            // LKR per tile
    skirting_len_ft: undefined,    // optional; default = ceil(20% of area)
    user_labor_rate_sqft: undefined, // optional; overrides min/max if > 0
  });

  const rep: Report | null = useMemo(() => {
    try {
      return computeReport(inp, sizes as any);
    } catch {
      return null;
    }
  }, [inp]);
    ...
    <main className="home">
      ...
      <section className="section card">
        <div style={{display:'grid', gap:12, gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))'}}>
          ...
          <div>
            <label>Labor Rate (LKR / sqft) <span style={{color:'var(--muted)'}}>(optional, overrides)</span></label>
      ...
      <section className="section card" ref={reportRef}>
        ...
        {!rep ? (
          <p style={{opacity:.7}}>Enter area/length×width, pick a tile size, and set price per tile to see the report.</p>
        ) : (
          <div style={{display:'grid', gap:12, gridTemplateColumns:'repeat(auto-fit, minmax(260px, 1fr))'}}>
            ...
            <div className="card" style={{boxShadow:'none', padding:0}}>
              ...
              <table className="table"><tbody>
                ...
                <tr><td>Total Tiles (5% wastage)</td><td><b>{rep.total_tiles}</b></td></tr>
              </tbody></table>
            </div>
            ...
            <div className="card" style={{boxShadow:'none', padding:0}}>
              ...
              <table className="table"><tbody>
                <tr><td>Floor Tiles</td><td><b>{rep.floor_tiles}</b></td><td>-</td></tr>
                <tr><td>Skirting Tiles</td><td><b>{rep.skirting_tiles}</b></td><td>-</td></tr>
                <tr><td><b>Total Tiles</b></td><td><b>{rep.materials.tiles_total_qty}</b></td><td><b>LKR {rep.materials.tiles_cost.toLocaleString()}</b></td></tr>
                <tr><td>Adhesive (25kg)</td><td>{rep.materials.adhesiveMin_bags} – {rep.materials.adhesiveMax_bags}</td><td>LKR {rep.materials.adhesiveMin_cost.toLocaleString()} – {rep.materials.adhesiveMax_cost.toLocaleString()}</td></tr>
                <tr><td>Clips (100 pcs)</td><td>{rep.materials.clips_qty}</td><td>LKR {rep.materials.clips_cost.toLocaleString()}</td></tr>
                <tr><td>Grout (1kg)</td><td>{rep.materials.grout_kg}</td><td>LKR {rep.materials.grout_cost.toLocaleString()}</td></tr>
              </tbody></table>
            </div>
            ...
            <div className="card" style={{boxShadow:'none', padding:0}}>
              ...
              <table className="table"><tbody>
                <tr><td>Cement (50kg)</td><td>{rep.materials.cementMin_bags} – {rep.materials.cementMax_bags}</td><td>LKR {rep.materials.cementMin_cost.toLocaleString()} – {rep.materials.cementMax_cost.toLocaleString()}</td></tr>
                <tr><td>Sand (1 cube)</td><td>{rep.materials.sandMin_cubes} – {rep.materials.sandMax_cubes}</td><td>LKR {rep.materials.sandMin_cost.toLocaleString()} – {rep.materials.sandMax_cost.toLocaleString()}</td></tr>
              </tbody></table>
            </div>
