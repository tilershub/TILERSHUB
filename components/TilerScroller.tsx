...
export default function TilerScroller({ tilers }: { tilers: Tiler[] }) {
  ...
  const badgeTone = (label?: string) => {
    switch (label) {
      ...
      case 'TILERSHUB Certified':  return 'border-blue-200 bg-blue-50 text-blue-900';
      case 'Verified':             return 'border-sky-200 bg-sky-50 text-sky-900';
  ...
  const BadgeIcon = ({ label }: { label?: string }) => {
    ...
    switch (label) {
      ...
      case 'Professional Tiler':  // briefcase
      ...
      case 'TILERSHUB Certified': // shield-check
          ...
          <article key={t.id}
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="mt-2 flex flex-col items-center leading-tight">
                <div className="tracking-[2px] text-sm opacity-75">{stars(t.rating)}</div>
              ...
              {main && (
                <span
                  className={`mt-2 inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full border ${badgeTone(main)}`}
                  aria-label={main}
                  title={main}
                >
            ...
            {/* RIGHT: details + extra badges + tags + actions */}
            <div>
              ...
              {!!rest.length && (
                <div className="flex flex-wrap gap-2 mb-2">
                  {rest.map(b => (
                    <span key={b} className={`inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full border ${badgeTone(b)}`}>
                      <BadgeIcon label={b} />
                      {b}
                    </span>
                  ))}
